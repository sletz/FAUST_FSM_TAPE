...
with {
    // Quantize bias_mode (original thresholds: <0.5, <1.5, ..., else)
    mode = int(bias_mode + 0.5);   // ideally 0..9

    // Clock for branch i (1 if active, 0 otherwise)
    clk(i) = (mode == i);

    // Generic loopK definition
    loopK(H, K) = (loop ~ (mem, mem)) : ba.selector(2, 3)
        with {
            loop(recM, recH) = (recM, recH, H) : K;
        };

    // Mode-indexed loop selection
    loop(0, H) = loopK(H, ja_loop_k28);
    loop(1, H) = loopK(H, ja_loop_k45);
    loop(2, H) = loopK(H, ja_loop_k63);
    loop(3, H) = loopK(H, ja_loop_k99);
    loop(4, H) = loopK(H, ja_loop_k121);
    loop(5, H) = loopK(H, ja_loop_k187);
    loop(6, H) = loopK(H, ja_loop_k253);
    loop(7, H) = loopK(H, ja_loop_k495);
    loop(8, H) = loopK(H, ja_loop_k1045);
    loop(9, H) = loopK(H, ja_loop_k2101);
};
